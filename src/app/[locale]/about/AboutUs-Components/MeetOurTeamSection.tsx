    ...
    <section id='OurTeam' className='min-h-screen flex flex-col  items-center p-8 pl-9 pr-9 bg-[#fbfbfbd9]'>
      ...
      <div className='h-3/4 w-full  flex xl:flex-row lg:flex-row md:flex-row 2sm:flex-col justify-center items-center '>
        <div className='w-2/4	 flex justify-end items-end  '>
          ...
          {newperson.photo ? (
                  <Image src={newperson.photo} alt='photo' className={`w-full h-full xl:rounded-[0rem] lg:rounded-[0rem] md:rounded-[0rem] 2sm:rounded-[10rem]  ${styles.tv} `} />
                ) : (
                  <span>No photo available</span>
                )}
          </div>
        </div>
        <div className=' xl:block lg:block md:block 2sm:hidden   xl:h-[30rem] lg:h-[25rem] md:h-[25rem] 2sm:h-[15rem] border-l-[0.2rem] bg xl:ml-[10rem] lg:ml-[6rem] md:ml-[3rem]    border-[#287195] mt-[2rem]'></div>
        <div className='w-2/4 xl:ml-[10rem] lg:ml-[1rem] lg:mr-[9rem] md:mr-[1rem] 2sm:mr-[11rem] 2sm:mt-[2rem]	 h-full  flex justify-start items-start  '>
          <div className='flex flex-col justify-start items-center'>
            ...
            <div className='w-[10rem]  border-[0.1rem]  border-[#287195] mt-[0.6rem] mb-[0.5rem]'></div>
            ...
            <div className='flex w-[20rem] mt-[1rem] h-[4rem] justify-center items-center '>
              <div className='xl:w-[3rem] xl:h-[3rem] lg:w-[3rem] lg:h-[3rem] md:w-[2.5rem] md:h-[2.5rem] 2sm:h-[2rem] 2sm:w-[2rem] rounded-[5rem] bg-slate-100 hover:bg-[#c2d8ff] flex justify-center items-center border-[0.1rem] border-black'>
              ...
              <div className='xl:w-[3rem] xl:h-[3rem] lg:w-[3rem] lg:h-[3rem] md:w-[2.5rem] md:h-[2.5rem] ml-[2rem] 2sm:h-[2rem] 2sm:w-[2rem] rounded-[5rem] bg-slate-100 hover:bg-[#c2d8ff] flex justify-center items-center border-[0.1rem] border-black'>
      ...
      <div className='xl:flex lg:flex md:flex 2sm:grid 2sm:grid-cols-2 2sm:grid-rows-2 xl:gap-[0rem] lg:gap-[0rem] md:gap-[0rem] 2sm:gap-[3rem] mt-[3rem]'>
            {object.map((person: Object)=>(
                 <div key={person.id} className={`xl:w-[13rem] xl:h-[17rem] lg:w-[10.5rem] lg:h-[15rem] md:w-[7.8rem] 2sm:w-[4rem] flex flex-col justify-center  items-center ${newperson.id===person.id?' bg-white border-[0.1rem] border-[#287195] rounded-[3rem] 	':''} `}>
                 <div className='bg-bla xl:h-[9rem] xl:w-[9rem] lg:h-[9rem] lg:w-[9rem] md:h-[6rem] md:w-[6rem] 2sm:h-[5rem] 2sm:w-[5rem] rounded-[13rem]'>
                 {person.photo ? (
                  <Image src={person.photo} alt='photo' className='w-full h-full rounded-[5rem]' />
                ) : (
                  <span>No photo available</span>
                )}
                 </div>
                 <div className='bg-secondary xl:w-[7rem] xl:h-[2.0rem] lg:w-[7rem] lg:h-[2.0rem] md:w-[7rem] md:h-[2.0rem] 2sm:w-[4rem] 2sm:h-[2.0rem] rounded-[1.7rem] flex justify-center mt-[1rem]'><button onClick={() => person.id !==undefined && changehtml(person.id)}  className='text-white font-extrabold'>{person.name}</button></div>
             </div>
